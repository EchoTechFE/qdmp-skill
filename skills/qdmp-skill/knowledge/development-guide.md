...
非交互环境请预先设置 `QDMP_TOKEN`；未设置时 CLI 应输出错误并退出，不等待输入。登录成功后再上传：

```bash
qdmp-cli upload -d "新增商品筛选并优化加载体验"
```

交互登录不能放入后台任务或与构建、上传串联执行；账号密码方式输入时密码不回显属于正常现象。如果当前执行工具不能让用户直接操作终端，应让用户在自己的终端完成登录，再执行 `qdmp getMe` 验证；不要在聊天中索取账号或密码。
